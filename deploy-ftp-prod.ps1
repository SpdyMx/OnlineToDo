...
$ProfileName = 'prod'
$DefaultLocalPath = '.\\prod'
$DefaultConfigFile = '.\\deploy-ftp-prod.config.json'
$UseMinify = $true
$UseObfuscation = $true
$UsePhpObfuscation = $true
...
function Obfuscate-PhpFiles {
    param([Parameter(Mandatory = $true)][string]$TargetDir)

    Write-Host "==> [$ProfileName] Obfuscating PHP files" -ForegroundColor Yellow

    $phpFiles = Get-ChildItem -Path $TargetDir -Recurse -File -Filter *.php | Where-Object {
        $_.FullName -notmatch '\\vendor\\' -and
        $_.FullName -notmatch '\\var\\' -and
        $_.FullName -notmatch '\\logs\\' -and
        $_.FullName -notmatch '\\tests\\'
    }

    foreach ($phpFile in $phpFiles) {
        $obfuscated = & php -w $phpFile.FullName
        if ($LASTEXITCODE -ne 0) {
            throw "PHP obfuscation failed for file: $($phpFile.FullName)"
        }

        Set-Content -Path $phpFile.FullName -Value $obfuscated -Encoding UTF8 -NoNewline
    }
}
...
function Prepare-DeploymentPackage {
    ...
    if ($UsePhpObfuscation) {
        Obfuscate-PhpFiles -TargetDir $targetDir
    }

    Write-Host "==> [$ProfileName] Building frontend (minified)" -ForegroundColor Yellow
    ...
    try {
        ...
        if ($UseObfuscation) {
            $assetsDir = Join-Path $targetDir 'public/assets'
            if (Test-Path -Path $assetsDir) {
                Write-Host "==> [$ProfileName] Obfuscating frontend assets" -ForegroundColor Yellow
                npx --yes javascript-obfuscator "$assetsDir" --output "$assetsDir" --compact true --self-defending true --identifier-names-generator hexadecimal --string-array true --string-array-encoding base64 --rename-globals false
            }
        }
    }
    finally {
